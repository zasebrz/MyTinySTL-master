...
{
  auto d1 = mystl::distance(first1, last1);//计算两个迭代器之间的距离
  ...
  {//只要找到序列2的最后一个元素，就退出
    ...
    {
      if (d1 == d2)
      {//如果此时序列1剩余长度d1和d2相等，但是当前元素不相同，那么一定无法找到序列2
        return last1;
      }
      else
      {
        current1 = ++first1;//为什么要递增f1？直接递增cur1不就行了？当然不行，需要有一个指针指向序列2的首元素
        current2 = first2;//仍然从头查找
        --d1;//剩余长度减少
      }
    }
  }
  return first1;//返回首次出现点
}
       ...
       ForwardIter2 first2, ForwardIter2 last2, Compared comp)
{
  ...
  while (current2 != last2)
  {
    if (comp(*current1, *current2))//不管是迭代器还是指针都可以取值
    {
      ++current1;
      ++current2;
    }
...
ForwardIter
search_n(ForwardIter first, ForwardIter last, Size n, const T& value)//Size可能是short，int，long，long long
{
  if (n <= 0)
  {
    return first;
  ...
  {
    first = mystl::find(first, last, value);//找到第一个元素，不加mystl::也是一样的，因为这两个模板函数属于同一个命名空间
    while (first != last)
    {
      auto m = n - 1;
      auto i = first;
      ...
      {
        first = mystl::find(i, last, value);//否则再往后找下一个元素，重复上面的过程，注意这里的起始点就变成了i
      }
    }
    return last;//上面没有返回的话，就说明没找到，返回last
  }
}
...
/*****************************************************************************************/
// find_end
// 在[first1, last1)区间中查找[first2, last2)最后一次出现的首元素的地方，若不存在返回 last1
/*****************************************************************************************/
...
find_end_dispatch(ForwardIter1 first1, ForwardIter1 last1,
                  ForwardIter2 first2, ForwardIter2 last2,
                  forward_iterator_tag, forward_iterator_tag)//前向迭代器类，
{
  if (first2 == last2)
  {
    return last1;
                  ...
                  bidirectional_iterator_tag, bidirectional_iterator_tag)
{
  ...
  //reverse_iterator是模板类，使用的时候必须申明实例化参数，也就是BidirectionalIter1，
  //所以reverse_iterator<BidirectionalIter1>就代表一个类
  //然后再给这个类起个别名叫做 reviter1
  typedef reverse_iterator<BidirectionalIter2> reviter2;
  reviter1 rlast1(first1);//用类的别名申明一个类，first1是构造函数的参数，记住reviter1是一个类，有自己的构造函数
  //正向的first1就变成了反向的last1
  reviter2 rlast2(first2);
  reviter1 rresult = mystl::search(reviter1(last1), rlast1, reviter2(last2), rlast2);//反向查找
  if (rresult == rlast1)
  {
    return last1;//没找到？
  }
  ...
  {
    auto result = rresult.base();//找到以后要返回正向的迭代器，所以先用base得到对应的前向迭代器
    mystl::advance(result, -mystl::distance(first2, last2));//因为要返回的是最后一次出现的首元素位置，而result此时的位置是最后一次出现的尾元素位置
                                                            //所以要往前移动
    return result;
  }
}
         ...
         ForwardIter2 first2, ForwardIter2 last2)
{
  ...
  return mystl::find_end_dispatch(first1, last1, first2, last2, Category1(), Category2());//实例化以后才能传递
}
...
InputIter
find_first_of(InputIter first1, InputIter last1,//只读不写就使用输入迭代器
...
Function for_each(InputIter first, InputIter last, Function f)//迭代器不会对元素进行写操作，只会通过f操作
{
  for (; first != last; ++first)
  {
    f(*first);
...
/*****************************************************************************************/
// lower_bound
// 在[first, last)中查找第一个大于等于 value 的元素，并返回指向它的迭代器，若没有则返回 last
/*****************************************************************************************/
// lbound_dispatch 的 forward_iterator_tag 版本
template <class ForwardIter, class T>
ForwardIter
lbound_dispatch(ForwardIter first, ForwardIter last,
                const T& value, forward_iterator_tag)//forward_iterator_tag是一个标签，用来表示这个函数时是前向迭代器重载版本
{
  ...
  {//二分查找
    half = len >> 1;
    middle = first;
    mystl::advance(middle, half);//迭代器走到中间
    if (*middle < value)
    {//序列是有序的
      first = middle;
      ++first;
      len = len - half - 1;//后半段中查找
    }
    else
    {
      len = half;//前半段中查找
...
{
  return mystl::lbound_dispatch(first, last, value, iterator_category(first));//萃取迭代器的类型，和之前结果是一样的
}
...
{
  auto i = mystl::lower_bound(first, last, value);//先找到大于等于它的第一个元素迭代器
...
bool binary_search(ForwardIter first, ForwardIter last, const T& value, Compared comp)
{
  ...
  return i != last && !comp(value, *i);//这里有问题
}
                ...
                const T& value, forward_iterator_tag)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = mystl::lower_bound(first, last, value);//已经确定了有相等的元素
      mystl::advance(first, len);//将first前移用来表示当前序列的尾元素
      right = mystl::upper_bound(++middle, first, value);//这里不能用last，因为last从来没有变过，它代表原始序列的尾元素
...
bool is_heap(RandomIter first, RandomIter last)
{
  ...
  {
    if (first[parent] < first[child])//由于first传入的是自定义的迭代器，底层是指针，所以可以用这种形式，如果是std的迭代器则不行
      return false;
    if ((child & 1) == 0)
      ++parent;
  }
...
/*****************************************************************************************/
// swap_ranges
// 将序列1[first1, last1)所有元素和序列2从 first2 开始的元素进行交换
                      ...
                      bidirectional_iterator_tag)
{
  while (true)
  {
    if (first == last || first == --last)//这里比较的first是已经递增之后的，而last还没有递减，仍指向一个已经交换过位置的元素
                                         //如果两个相等，则说明元素个数是偶数，此时在递减last，如果两个相等，说明此时还没有
                                         //交换位置的元素只剩下一个了，不需要交换
      return;
...
{
  return mystl::copy(first, middle, mystl::copy(middle, last, result));//先拷贝后半段，再拷贝前半段，相当于旋转拷贝
}
...
{
  auto i = last;
  if (first == last || first == --i)//为空或者只有一个元素，注意这里i已经递减了
    return false;
  ...
  {
    auto ii = i;
    if (*--i < *ii)//相邻元素判断，ii就是j，i就是i
    {                 // 找到了从右往左数第一对前数小于后数的元素对
      auto j = last;//开始步骤2
      while (!(*i < *--j)) {}//空循环是什么意思？只是为了递减j，j就是k
      mystl::iter_swap(i, j);       // 交换 i，j 所指元素
      mystl::reverse(ii, last);     // 将 ii 之后的所有元素反转
      return true;
    }
    if (i == first)
    {//否则当i递减到first的时候，说明整个序列都是降序，直接逆转
      ...
      OutputIter result)
{
  ...
  return mystl::copy(first2, last2, mystl::copy(first1, last1, result));//注意这里的拷贝，因为上面while循环要求两个指针都不为空
  //所以肯定会有一个序列某些元素没有拷贝进去（即使两个序列长度相同），这样的话假设序列1没有完成拷贝，则copy(first1, last1, result)
  //将剩余没有拷贝的元素拷贝过去，注意first指针也是在循环里变化的，而序列2完成拷贝了，则copy(first2, last2）不起作用，最终
  //完成所有元素的拷贝
}
